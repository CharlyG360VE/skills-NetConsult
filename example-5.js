...
const example5 = ( users ) => {
  ...
  for ( const user of users ) {
    ...
    if ( user.car === true ) {
      userHasCar++;
      ageWithCar = age + user.age;
    };
  };
  user.average = age / users.length;
  user.hasCar = userHasCar;
  user.averageWithCar = ageWithCar / user.hasCar;
  return user;
};

console.log('---- Completed skill - EXAMPLE 5 --- ', example5( example4( companies ) ) );
